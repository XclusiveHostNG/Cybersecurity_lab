...
$id = (int)($_GET['id'] ?? 0);
try {
    $stmt = db()->prepare("SELECT * FROM events WHERE id = ? AND created_by_type='aspirant' AND created_by = ?");
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white border-0">
        <h5 class="mb-0"><i class="fa fa-edit me-2 text-primary"></i>Edit Event</h5>
    </div>
    <div class="card-body">
        <?php if (!$event): ?>
            <div class="alert alert-danger">Event not found.</div>
        <?php else: ?>
            <form action="/handlers/aspirant/event-edit-handler.php" method="post" class="row g-3">
                ...
                <div class="col-12">
                    <button type="submit" class="btn btn-primary"><i class="fa fa-save me-2"></i>Update Event</button>
                </div>
            </form>
        <?php endif; ?>
    </div>
